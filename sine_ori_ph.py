...
stim_info.columns = ['stim_on','stim_dur','phase','dir']; 
...
phs = sorted(stim_info['phase'].unique()); 
total_condnums = len(dirs)*len(phs); 

StimResp = []
for i in range(total_condnums):
    StimResp.append(dict()); 
    StimResp[i]['dir'] = np.nan; 
    StimResp[i]['phase'] = np.nan; 
    StimResp[i]['trials'] = []; 
    StimResp[i]['stim_on'] = []; 
    StimResp[i]['stim_off'] = [];    
    StimResp[i]['neurons'] = [];  
...
for d, dir in enumerate(dirs):
    for p, ph in enumerate(phs):
        ...
        StimResp[condnum]['phase'] = ph; 

        trials = np.where((stim_info['dir']==dir) & (stim_info['phase']==ph))[0]
...
for i in range(nCells):
    ...
    # dir, phase tuning
    ax2.clear();         
    respNow = mResp[:,i].reshape(len(dirs),len(phs)).T; 
    for p in range(len(phs)):
        ax2.plot(dirs,respNow[p,:],'-o',label=f'phase = {phs[p]}'); 
    ...
    # phase tuning
    ax4.clear();         
    ax4.plot(phs,np.mean(respNow,axis=1),'-ko'); 
...
if max(dirs)>180:
    ...
    for i in range(int(len(dirs))):
        ...
        ori_now = np.where((stim_info['dir']==dir1))[0]; 
...
ax1 = plt.subplot(2,2,1); 
ax3 = plt.subplot(2,2,3); 
cool_colors1 = plt.get_cmap('cool',len(phs)); 

ax2 = plt.subplot(2,2,2); 
ax4 = plt.subplot(2,2,4); 
rainbow_colors1 = plt.get_cmap('hsv',len(phs)); 

ax1.imshow(im);  # Cells segmented in the image
for i in range(nCells):
    respNow = mResp[:,i].reshape(len(dirs),len(phs)).T;     
    ph_resp = np.mean(respNow, axis=1); 

    ph_idx = np.where(ph_resp==np.max(ph_resp))[0]; 

    ypix = stats[i]['ypix'][~stats[i]['overlap']]; 
    xpix = stats[i]['xpix'][~stats[i]['overlap']]; 

    color_now = cool_colors1(ph_idx/(len(phs)-1)); 
    ax1.plot(xpix,ypix,color=color_now[:3], alpha=0.8); 
    ax1.text(np.mean(xpix),np.mean(ypix),f'{i}'); 
ax1.set_title('Phase map')

for i in np.arange(len(phs)):
    color_now = cool_colors1(i/(len(phs)-1)); 
    ax3.plot(i, 1, 'o', 
             color=color_now[:3], markersize=20)
    ax3.text(i, 0.98, f'{phs[i]}')
ax3.set_axis_off()
...
for i in range(nCells):
    ...
    color_now = rainbow_colors1(ph_idx/(len(phs)-1)); 
    ax2.plot(xpix,ypix,color=color_now[:3], alpha=0.8); 
    ax2.text(np.mean(xpix),np.mean(ypix),f'{i}'); 
ax2.set_title('Phase map')

for i in np.arange(len(phs)):
    color_now = rainbow_colors1(i/(len(phs)-1)); 
    ax4.plot(i, 1, 'o', 
             color=color_now[:3], markersize=20)
    ax4.text(i, 0.98, f'{phs[i]}')
ax4.set_axis_off()

plt.tight_layout(); 
plt.savefig(suite2p_path+expName+'_Phase_map.pdf')
