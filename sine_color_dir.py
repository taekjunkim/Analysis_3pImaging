...
stim_info.columns = ['stim_on','stim_dur','color_flag','dir']; 
...
cfs = sorted(stim_info['color_flag'].unique()); 
total_condnums = len(dirs)*len(cfs); 

StimResp = []
for i in range(total_condnums):
    StimResp.append(dict()); 
    StimResp[i]['dir'] = np.nan; 
    StimResp[i]['color_flag'] = np.nan; 
    StimResp[i]['trials'] = []; 
    StimResp[i]['stim_on'] = []; 
    StimResp[i]['stim_off'] = [];    
    StimResp[i]['neurons'] = [];  
...
for d, dir in enumerate(dirs):
    for c, cf in enumerate(cfs):
        ...
        StimResp[condnum]['color_flag'] = cf; 

        trials = np.where((stim_info['dir']==dir) & (stim_info['color_flag']==cf))[0]
...
for i in range(nCells):
    ...
    # dir, color_flag tuning
    ax2.clear();         
    respNow = mResp[:,i].reshape(len(dirs),len(cfs)).T; 
    for c in range(len(cfs)):
        ax2.plot(dirs,respNow[c,:],'-o',label=f'color_flag = {cfs[c]}'); 
    ...
    # color_flag tuning
    ax4.clear();         
    ax4.plot(cfs,np.mean(respNow,axis=1),'-ko'); 
    ax4.set_xlabel('Color_flag'); 
...
ax1 = plt.subplot(2,2,1); 
ax3 = plt.subplot(2,2,3); 
cool_colors1 = plt.get_cmap('cool',len(cfs)); 

ax2 = plt.subplot(2,2,2); 
ax4 = plt.subplot(2,2,4); 
rainbow_colors1 = plt.get_cmap('hsv',len(cfs)); 

ax1.imshow(im);  # Cells segmented in the image
for i in range(nCells):
    respNow = mResp[:,i].reshape(len(dirs),len(cfs)).T;     
    cf_resp = np.mean(respNow, axis=1); 

    cf_idx = np.where(cf_resp==np.max(cf_resp))[0]; 

    ypix = stats[i]['ypix'][~stats[i]['overlap']]; 
    xpix = stats[i]['xpix'][~stats[i]['overlap']]; 

    color_now = cool_colors1(cf_idx/(len(cfs)-1)); 
    ax1.plot(xpix,ypix,color=color_now[:3], alpha=0.8); 
    ax1.text(np.mean(xpix),np.mean(ypix),f'{i}'); 
ax1.set_title('Color_flag map')

for i in np.arange(len(cfs)):
    color_now = cool_colors1(i/(len(cfs)-1)); 
    ax3.plot(i, 1, 'o', 
             color=color_now[:3], markersize=20)
    ax3.text(i, 0.98, f'{cfs[i]}')
ax3.set_axis_off()
...
for i in range(nCells):
    ...
    color_now = rainbow_colors1(cf_idx/(len(cfs)-1)); 
    ax2.plot(xpix,ypix,color=color_now[:3], alpha=0.8); 
    ax2.text(np.mean(xpix),np.mean(ypix),f'{i}'); 
ax2.set_title('Color_flag map')

for i in np.arange(len(cfs)):
    color_now = rainbow_colors1(i/(len(cfs)-1)); 
    ax4.plot(i, 1, 'o', 
             color=color_now[:3], markersize=20)
    ax4.text(i, 0.98, f'{cfs[i]}')
ax4.set_axis_off()

plt.tight_layout(); 
plt.savefig(suite2p_path+expName+'_ColorFlag_map.pdf')
